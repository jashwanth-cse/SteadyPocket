      ...
      <div className="grid grid-cols-1 gap-4">
        {alerts
          ...
          <motion.div 
            ...
            className="bg-[#111111] border border-white/5 rounded-3xl p-5 flex flex-col xl:flex-row xl:items-center gap-6 group hover:border-emerald-500/20 transition-all"
          >
            ...
            <div className="flex items-center gap-4 flex-1 min-w-[300px]">
              ...
              <div className="overflow-hidden">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-bold text-white truncate">{getUserName((alert as any).user_id || (alert as any).userId)}</span>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-white/5 text-neutral-500 whitespace-nowrap">
                    {getAlertBadge(alert.alert_type || 'general')}
                  </span>
                </div>
                <p className="text-[11px] text-neutral-600 flex items-center gap-1 font-mono">
            ...
            {/* Middle: Metrics & Status (Centered Grid) */}
            <div className="flex items-center justify-between xl:justify-start gap-12 lg:gap-24 xl:w-1/3">
              ...
              <div className="min-w-[100px]">
                <p className="text-[10px] text-neutral-500 uppercase tracking-widest font-bold mb-1 opacity-60">Current Status</p>
            ...
            <div className="flex items-center gap-2 ml-auto">
              <button 
                ...
                className={`flex items-center gap-2 px-3 py-2 bg-white/5 text-neutral-400 border border-white/5 rounded-xl hover:bg-neutral-500/10 hover:text-neutral-300 transition-all ${processingId === alert.id ? 'opacity-50 cursor-not-allowed' : ''}`} 
                title="Ignore Alert"
              >
                <Ban className={`w-4 h-4 ${processingId === alert.id && processingId === 'Ignored' ? 'animate-spin' : ''}`} />
              ...
              <button 
                ...
                disabled={processingId === alert.id}
                className={`flex items-center gap-2 px-3 py-2 bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 rounded-xl hover:bg-yellow-500/20 transition-all ${processingId === alert.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                title="Investigate"
              >
                <ShieldAlert className={`w-4 h-4 ${processingId === alert.id && processingId === 'Under_review' ? 'animate-spin' : ''}`} />
              ...
              <button 
                ...
                disabled={processingId === alert.id}
                className={`flex items-center gap-2 px-3 py-2 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-xl hover:bg-emerald-500/20 transition-all shadow-lg shadow-emerald-500/5 ${processingId === alert.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                title="Mark Resolved"
              >
                <CheckCircle className={`w-4 h-4 ${processingId === alert.id && processingId === 'Resolved' ? 'animate-spin' : ''}`} />
