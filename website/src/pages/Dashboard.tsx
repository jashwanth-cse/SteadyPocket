...
export default function Dashboard() {
    ...
    for (let i = 0; i < days; i++) {
      ...
      const dayPayouts = payouts.filter((p) => {
        const ts = p.timestamp?.toDate
          ? p.timestamp.toDate()
          : new Date(p.timestamp || 0);
        return ts >= dayStart && ts <= dayEnd;
      });

      const dayPremium = policies
        .filter((p) => {
      ...
      result.push({
        name:
          days === 1
            ...
            : dateStr,
        premium: dayPremium,
        payouts: dayPayouts.reduce(
          (sum, p) => sum + (p.amount || (p as any).total_payout || 0),
          0,
        ),
      });
    }
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: stats.affectedRiders.toLocaleString(),
      icon: Users,
      color: "purple",
      trend: "+18%",
    },
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-neutral-500">
            Real-time monitoring of Steady Pocket ecosystem.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        {isLoading && riders.length === 0
          ...
          : statCards.map((stat, i) => (
              ...
              >
                <div
                  className={`absolute top-0 right-0 w-24 h-24 bg-${stat.color}-500/5 rounded-full -mr-8 -mt-8 transition-transform group-hover:scale-150 duration-500`}
                />
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div
                    className={`flex items-center gap-1 text-xs font-bold ${stat.trend.startsWith("+") ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {stat.trend}
