...
exports.getAllPayouts = async (req, res) => {
  try {
    ...
    const payoutsSnapshot = await db.collection('payouts').orderBy('timestamp', 'desc').get();
...
exports.updatePayoutStatus = async (req, res) => {
  ...
  const validStatuses = ['approved', 'rejected', 'pending', 'failed', 'Processing', 'Completed'];
  ...
  try {
    ...
    if (payoutData.status === 'approved' && status !== 'approved') {
      return res.status(400).json({ error: 'Cannot modify a payout that has already been approved' });
    }
